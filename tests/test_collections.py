...
from spillway.collections import (
    Feature,
    FeatureCollection,
    LayerCollection,
    LinkedCRS,
...
class FeatureTestCase(SimpleTestCase):
    ...
    def test_dict(self):
        self.assertEqual(
            Feature(**{"geometry": self.geom, "name": "atlantic"}),
            {
                "geometry": self.geom,
                "type": "Feature",
...
class LayerCollectionTestCase(SimpleTestCase):
    ...
    def test_as_feature(self):
        self.assertIsInstance(as_feature(dict(self.lc["layer"])), FeatureCollection)
